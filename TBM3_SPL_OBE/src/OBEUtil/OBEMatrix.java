...
public class OBEMatrix {
     ...
     *
     * @param values matrix values that representate the coefficient for the
     * respected variables.
     * @param constants
     */
     ...
     * Constructor for this class.
     *
     * @param values Matrix object that holds the equation's value.
     * @param constants Constants that remined behind the equal symbol.
     */
     ...
     *
     * @param target Target's index location
     * @param substractor substractor index location
     * @param scalar Scalar value that will be multiplied to the subtractor
     * before the subtraction
     * @throws ArrayIndexOutOfBoundsException When the index value are not in
     * bounds, this exception will be thrown
     */
     ...
     * @param scalar Scalar values that will be multiplied
     * @throws ArrayIndexOutOfBoundsException When the index value are not in
     * bounds, this exception will be thrown
     */
    ...
    public boolean isMEBT() {
        ...
        for (int i = 0; i < values.length && result; i++) {
            for (int j = 0; j < values.length && result; j++) {
     ...
     * @throws UnableToSolveException then the equation cannot be solved, then
     ...
     * has only 1 solution, and lastly 2 if it cannot be determined.
     */
    public int detectSolution() {
        int hasSolution = detectHasSolution();
        if(hasSolution == 1)
            return 0;
     ...
     * @return
     */
    private int detectHasSolution() {
        double hasil = 0;
        boolean infiniteFlag=false;
        for (int i = 0; i < values.length; i++, hasil=0) {
            for (int j = 0; j < values.length; j++) {
                hasil += values[i][j];
            }
            if (hasil == 0 && constants[i] != 0) {
                return 1;
            } else if(hasil == 0 && constants[i] == 0)
                infiniteFlag=true;
        }
        return (infiniteFlag?-1:0);
    }
     ...
     */
    private boolean detectConsistency() {
        resync();
        for (int j = 0; j < values.length - 1; j++) {
            for (int i = j; i < values.length; i++) {
                if (!detectRowRatioConsistant(i, j)) {
                    return false;
                }
            }
        }
    ...
    private boolean detectRowRatioConsistant(int rowA, int rowB) {
        ...
        return ((constants[rowA]/constants[rowB]) == ratio);
    }
